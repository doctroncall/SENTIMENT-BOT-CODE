...
def main() -> None:
    ...
    # ============================================================
    # HOME TAB - ALL IMPORTANT INFO IN ONE PLACE
    # ============================================================
    with tab_home:
        st.header("🏠 Trading Bot Dashboard - Complete Overview")
        ...
        with col1:
            if st.button("▶️ Run Full Analysis", type="primary", use_container_width=True, key="home_run_analysis"):
        ...
        with col2:
            manual_symbol_home = st.text_input("Symbol", placeholder="GBPUSD", key="home_manual_symbol")
            if st.button("🎯 Analyze Symbol", use_container_width=True, key="home_analyze_symbol"):
                if manual_symbol_home.strip():
                    with st.spinner(f"Analyzing {manual_symbol_home}..."):
                        result, out, err = capture_output(dashboard.run_manual_analysis, manual_symbol_home.strip())
                    if err:
                        st.error(f"❌ Failed: {err}")
                    else:
                        st.success(f"✅ {manual_symbol_home} Done!")
                        st.rerun()
                else:
        ...
        with col4:
            if st.button("🔄 Run Retrain", type="secondary", use_container_width=True, key="home_retrain"):
                with st.spinner("Retraining model..."):
                    _, out, err = capture_output(dashboard.run_retrain)
                if err:
                    st.error(f"❌ Failed: {err}")
                else:
                    st.success("✅ Retrain Done!")
                    st.rerun()
        ...
        with col1:
            mt5_status = get_mt5_status(dashboard)
            if mt5_status['connected']:
                st.metric("MT5 Status", "🟢 Connected", delta="Online")
            else:
                st.metric("MT5 Status", "🔴 Offline", delta="Disconnected", delta_color="inverse")
                ...
                try:
                    ...
                    if "Verified" in df.columns:
                        ...
                        if not verified_df.empty:
                            correct = (verified_df["Verified"] == "✅ True").sum()
                            accuracy = (correct / len(verified_df)) * 100
                            st.metric("Accuracy", f"{accuracy:.1f}%", delta="Good" if accuracy >= 70 else "Improve")
                        else:
                ...
                if "Verified" in df.columns:
                    ...
                    if not verified_df.empty:
                        ...
                        with col_left:
                            ...
                            st.markdown(f"**{accuracy:.1f}%** accuracy ({correct}/{total} correct)")
                        ...
                        with col_right:
                            ...
                            if "Symbol" in verified_df.columns and len(verified_df["Symbol"].unique()) > 1:
                                ...
                                for symbol in sorted(verified_df["Symbol"].unique()):
                                    ...
                                    st.text(f"{symbol}: {symbol_acc:.0f}% ({symbol_correct}/{symbol_total})")
                    else:
                        st.info("📝 No verified predictions yet. Run verification after 1+ days.")
                else:
                    st.info("📝 No verification data available.")
            except Exception as e:
                st.error(f"❌ Error loading accuracy data: {e}")
        else:
            st.info("📝 No sentiment log found. Run an analysis first.")
        
        st.markdown("---")
        
        # ========== SECTION 4: CONFIGURATION INPUTS ==========
        ...
        with col1:
            symbols_input = st.text_area(
                ...
                height=100,
                help="Enter symbols like: GBPUSD, EURUSD, XAUUSD",
                key="home_symbols"
            )
        
        with col2:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("💾 Save Symbols", type="primary", use_container_width=True):
                symbols = parse_symbols(symbols_input)
                if symbols:
                    dashboard.symbols = symbols
                    st.success(f"✅ Saved {len(symbols)} symbols")
                    st.rerun()
                else:
                    st.warning("⚠️ No valid symbols")
        ...
        if os.path.exists(excel_file):
            try:
                ...
                if not df.empty:
                    ...
                    if "Weighted Score" in df.columns:
                        display_cols.append("Weighted Score")
                    
                    latest_df = df[display_cols].tail(15).sort_values("Date", ascending=False)
            ...
            st.info("📝 No data file found. Run an analysis to start.")
        ...
        if os.path.exists("reports"):
            files = sorted(
                [f for f in os.listdir("reports") if os.path.isfile(os.path.join("reports", f))],
                        ...
                        try:
                            with open(report_path, "rb") as fh:
                                st.download_button(
                                    ...
                                    mime="application/pdf" if report_file.endswith(".pdf") else "text/plain",
    ...
    # ============================================================
    # ANALYSIS TAB - RUNNING AND COMPLETED ANALYSES
    # ============================================================
    with tab_analysis:
